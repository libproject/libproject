...
Project::Project(Manager *manager, FileSetLoader *Loader)
  ...
  try {
    ...
    pathToNode = QString::fromStdString(loader->getPathToNode());
    ...
    FileNode *projectFileNode =
        new FileNode(Utils::FileName::fromString(pathToNode),
    ...
    for (const auto &file : filesToAdd) {
      files.append(QFileInfo(pathToNode).absolutePath() + QString("/") +
    ...
    if (loader->countSubprojects() > 0) {
      ...
      for (const auto &path : paths) {
        subprojects.append(QFileInfo(pathToNode).absolutePath() + QString("/") +
...
QString Project::displayName() const { return nameOfProject; }

IDocument *Project::document() const { return projectFile; }
...
// this is function which is called on demand to add files by other instances of
// plugin classes
bool Project::addFiles(const QStringList &filePaths) {
  ...
  for (const auto &path : filePaths) {
    ...
    if (QFileInfo(path).suffix() == QString("h") ||
        QFileInfo(path).suffix() == QString("cpp")) {
      fileNodes.push_back(new FileNode(
          Utils::FileName::fromString(path),
          QFileInfo(path).suffix() == QString("cpp") ? FileType::SourceType
                                                     : FileType::HeaderType,
          false));
    }
...
ProjectFile *Project::getProjectFile() const { return projectFile; }
