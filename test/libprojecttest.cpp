...
vector<FileSetLoader::Subprojects> pathsToAbnormalSubprojects = {
    ...
    {"project_files/testaddtonested/"
     "fortestwithonebrokenandoneregularsubprojects/normal.libproject",
     "project_files/testaddtonested/"
     "fortestwithonebrokenandoneregularsubprojects/broken.libproject"},
    {"project_files/testaddtonested/fortestwithtwobrokensubprojects/"
     "one.libproject",
     "project_files/testaddtonested/fortestwithtwobrokensubprojects/"
     "two.libproject"},
    {"project_files/testaddtonested/fortestwithoneregularandemptysubprojects/"
     "regular.libproject",
     "project_files/testaddtonested/fortestwithoneregularandemptysubprojects/"
     "empty.libproject"},
    {}};

string pathToNestedMainFile =
    R"(project_files/testaddtonested/mainproject_addnested.libproject)";
...
vector<FileSetLoader::Subprojects> incorrectPathsOfSubprojects = {
    ...
    // two non-existent
    {"subs/s1.libproject", "sub/nonexistent.libproject"},

    // pair of existent
    {"sub/s1.libproject", "sub/s1.libproject"},

    // one existent, one non-existent
    {"sub/s1.libproject", "sub/nonexistent.libproject"}
...
public:
  ...
  void TearDown() { TestSkeleton::TearDown(); }
};
...
TEST_F(TestRegularSingle, Attemt_to_save_file_without_changes) {
  ...
  ASSERT_EQ(fileToTestBeforeOperation.dump(4),
            fileToTestAfterOperation.dump(4));
}
...
public:
  ...
  FileSetLoader::Subprojects subprojectsFilesRef = {"sub/s1.libproject",
                                                    "sub/s2.libproject"};
...
public:
  FileSetLoader::Path pathToMainFileWhichIsTargetForAddBrokenSubproject =
      R"(project_files/testaddtosingle/mainproject_addbroken.libproject)";
  ...
  FileSetLoader::Path pathToBrokenSubproject =
      R"(project_files/testaddtosingle/broken/broken.libproject)";
  void SetUp() {
    pathToMainFile =
        R"(project_files/testaddtosingle/mainproject_addsingle.libproject)";
    pathToMainFileWhichIsTargetForAddBrokenSubproject =
        R"(project_files/testaddtosingle/mainproject_addbroken.libproject)";
    contentReference = {{"project", "there must be subprojects"},
...
TEST_F(TestAddRegularSubprojectsToSingle,
       Add_one_regular_single_which_already_cached) {
  ASSERT_THROW({
                 loader =
                     LoaderPtr(FileSetFactory::createFileSet(pathToMainFile));
                 loader->open();
                 loader->addSubproject(pathToOneRegularSingleSubproject);
                 loader->addSubproject(pathToOneRegularSingleSubproject);
               },
...
public:
  FileSetLoader::Path pathToPresentSubproject =
      R"(project_files/testaddtonested/presentsubproject/sub.libproject)";
  json contentReferenceWithPairOfNewRegularSubprojects = {
      {"project", "there must be subprojects"},
      {"files", {"main.cpp", "Test.h"}},
      {"subprojects",
       {"presentsubproject/sub.libproject",
  ...
  FileSetLoader::Subprojects pathsToPairOfRegularSubprojects = {
      "project_files/testaddtonested/fortestwithtworegularsubprojects/"
      "single.libproject",
      "project_files/testaddtonested/fortestwithtworegularsubprojects/"
      "nested.libproject"};
  void SetUp() {
    pathToMainFile = pathToNestedMainFile;
    contentReferenceWithSingle = {{"project", "there must be subprojects"},
                                  {"files", {"main.cpp", "Test.h"}},
                                  {"subprojects",
                                   {"presentsubproject/sub.libproject",
                                    "regular/normalsingle.libproject"}}};
    contentReferenceWithNested = {{"project", "there must be subprojects"},
                                  {"files", {"main.cpp", "Test.h"}},
                                  {"subprojects",
                                   {"presentsubproject/sub.libproject",
...
TEST_F(TestAddRegularSubprojectsToNested,
       Add_one_regular_single_which_already_cached) {
  ASSERT_THROW({
                 loader =
                     LoaderPtr(FileSetFactory::createFileSet(pathToMainFile));
...
TEST_F(TestAddRegularSubprojectsToNested,
       Add_one_regular_single_which_already_present_in_project_file) {
  ASSERT_THROW({
                 loader =
                     LoaderPtr(FileSetFactory::createFileSet(pathToMainFile));
                 loader->open();
...
TEST_F(TestAddRegularSubprojectsToNested,
       Add_already_cached_and_present_subprojects) {
  FileSetLoader::Subprojects subprojectsToAdd = {
      pathToOneRegularNestedSubproject, pathToPresentSubproject};
  ...
  ASSERT_THROW({ loader->addSubprojects(subprojectsToAdd); },
               FileSetRuntimeError);
  loader->save();
  ifstream i(pathToMainFile);
  fileToTest << i;
...
TEST_F(TestAddRegularSubprojectsToNested, Add_two_regular_subprojects) {
  ...
  ASSERT_EQ(contentReferenceWithPairOfNewRegularSubprojects.dump(4),
            fileToTest.dump(4));
}

class TestAddAbnormalSubprojects
    : public TestAddSubprojects,
...
INSTANTIATE_TEST_CASE_P(InstantiationOfTestAbnormalSubprojects,
                        TestAddAbnormalSubprojects,
                        ::testing::ValuesIn(pathsToAbnormalSubprojects));
TEST_P(TestAddAbnormalSubprojects,
       Set_of_attempts_to_add_abnormal_subprojects) {
  ASSERT_THROW({
                 loader =
                     LoaderPtr(FileSetFactory::createFileSet(pathToMainFile));
                 loader->open();
...
public:
  FileSetLoader::Path pathToProjectWithPairNormalSubprojects =
      R"(project_files/findsubprojectbypath/findsubproject.libproject)";
  FileSetLoader::Path pathToProjectWithoutSubprojects =
      R"(project_files/findsubprojectbypath/nosubproject.libproject)";
  string subproject1Name = "single_project_1";
  string subproject2Name = "single_project_2";
  FileSetLoader::Path pathToFirstSubproject =
      "project_files/findsubprojectbypath/sub/s1.libproject";
  FileSetLoader::Path pathToSecondSubproject =
      "project_files/findsubprojectbypath/sub/s2.libproject";
  FileSetLoader::Path pathToNonExistentSubproject =
      "project_files/findsubprojectbypath/sub/ne.libproject";
};

TEST_F(TestFindSubprojectByPath, Find_subprojects_by_paths) {
  loader = LoaderPtr(
      FileSetFactory::createFileSet(pathToProjectWithPairNormalSubprojects));
  loader->open();
  ASSERT_EQ(subproject1Name, loader->findSubprojectByPath(pathToFirstSubproject)
                                 ->getProjectName());
  ASSERT_EQ(
      subproject2Name,
      loader->findSubprojectByPath(pathToSecondSubproject)->getProjectName());
}
...
TEST_F(TestFindSubprojectByPath, Find_subproject_by_path_of_root_node) {
  ...
  loader->open();
  ASSERT_THROW(
      loader->findSubprojectByPath(pathToProjectWithPairNormalSubprojects),
      FileSetRuntimeError);
}
...
TEST_F(TestFindSubprojectByPath,
       Find_subproject_in_project_without_subprojects) {
  loader =
      LoaderPtr(FileSetFactory::createFileSet(pathToProjectWithoutSubprojects));
  loader->open();
  ASSERT_EQ(nullptr, loader->findSubprojectByPath(pathToNonExistentSubproject));
}
...
public:
  FileSetLoader::Path pathToProjectWithOneNormalSubprojects =
      R"(project_files/getsubprojectspaths/getpath.libproject)";
  FileSetLoader::Path pathToProjectWithPairNormalSubprojects =
      R"(project_files/getsubprojectspaths/getpaths.libproject)";
  FileSetLoader::Path pathToProjectWithoutSubprojects =
      R"(project_files/getsubprojectspaths/nosubprojects.libproject)";
  FileSetLoader::Subprojects pairOfPathsReference = {"sub/s1.libproject",
                                                     "sub/s2.libproject"};
  FileSetLoader::Subprojects pathReference = {"sub/s1.libproject"};
};

TEST_F(TestGetSubprojectsPaths, Get_paths_of_lone_subproject) {
  loader = LoaderPtr(
      FileSetFactory::createFileSet(pathToProjectWithOneNormalSubprojects));
  loader->open();
...
TEST_F(TestGetSubprojectsPaths,
       DISABLED_Get_subprojects_paths_of_project_without_them) {
  ASSERT_THROW({
                 loader = LoaderPtr(FileSetFactory::createFileSet(
                     pathToProjectWithoutSubprojects));
                 loader->open();
                 FileSetLoader::Subprojects paths =
                     loader->getSubprojectsPaths();
               },
               FileSetRuntimeError);
}

TEST_F(TestGetSubprojectsPaths,
       Get_paths_of_subprojects_on_not_loaded_interface) {
  ASSERT_THROW({
                 loader = LoaderPtr(FileSetFactory::createFileSet(
                     pathToProjectWithPairNormalSubprojects));
                 FileSetLoader::Subprojects paths =
                     loader->getSubprojectsPaths();
               },
               FileSetRuntimeError);
...
public:
  ...
  void SetUp() {
    ...
    in.close();
    contentBackup_oneSubproject =
        os_case_removing_subroject_from_array_of_two.str();
  ...
  void TearDown() {
    ...
    // reverting backups:
    ofstream out_case_removing_subroject_from_array_of_two(
    ...
    ofstream out_case_removing_lone_subproject(
...
class TestIncorrectSubprojectsRemoving
    : public TestProjectsWithFewSubprojects,
      public ::testing::WithParamInterface<FileSetLoader::Subprojects> {
public:
  void SetUp() {
...
INSTANTIATE_TEST_CASE_P(InstantiationOfTestIncorrectSubprojectsRemoving,
                        TestIncorrectSubprojectsRemoving,
                        ::testing::ValuesIn(incorrectPathsOfSubprojects));
TEST_P(TestIncorrectSubprojectsRemoving,
       Set_of_attempts_to_remove_subprojects_by_incorrect_paths) {
  ASSERT_THROW({
                 loader =
                     LoaderPtr(FileSetFactory::createFileSet(pathToMainFile));
                 loader->open();
